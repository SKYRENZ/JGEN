...
function MinistrySection() {
  ...
  return (
    <>
      <section id="ministries" className={`${SECTION_SCROLL_MARGIN} relative w-full overflow-hidden bg-black/20 ${SECTION_MIN_HEIGHT}`}>
        <div className={`flex ${SECTION_MIN_HEIGHT} flex-col lg:h-[calc(100svh-72px)] lg:flex-row`}>
          {ministries.map((ministry) => (
            <button
              key={ministry.title}
              type="button"
              onClick={() => setSelectedMinistry(ministry)}
              className="group relative min-h-64 flex-1 cursor-pointer overflow-hidden shadow-lg transition-all duration-500 hover:scale-[1.02] hover:brightness-110 hover:shadow-2xl hover:-translate-y-1 lg:min-h-0 lg:basis-0 lg:h-full"
              style={{
            ...
            >
              <div className="absolute inset-0 bg-linear-to-t from-black/60 via-black/30 to-transparent transition-all duration-300 ease-in-out group-hover:bg-black/20" />

              {ministry.title === 'Spiritual Ministries' ? (
                <div className="absolute inset-x-3 top-3 z-20 md:hidden">
                  <div className="rounded-2xl bg-black/45 px-4 py-3 text-center backdrop-blur-sm shadow-lg">
                    <p className="text-[10px] font-bold uppercase tracking-[0.32em] text-(--accent)">
                      Our Ministries
                    </p>
                    <h3 className="mt-2 font-display text-2xl leading-tight text-white drop-shadow-[0_8px_30px_rgba(0,0,0,0.9)]">
                      How We Serve
                    </h3>
              ...
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-end space-y-3 p-5 pb-8 text-center md:p-6 md:pb-12">
                ...
                <div className="opacity-0 scale-95 transition-all duration-300 group-hover:opacity-100 group-hover:scale-100">
        ...
        <div className="absolute left-1/2 top-10 z-10 hidden w-full max-w-2xl -translate-x-1/2 px-4 text-center md:block md:top-12">
